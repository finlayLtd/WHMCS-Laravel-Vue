  ...
  <section class="dashboard">
    <div class="container">
      ...
      <div class="sub-section">
        ...
        <div v-for="(kind, index) in os_kind" :key="index" :data-dist="kind" v-show="selected_os == kind">
          <div class="row server-item-wrapper os-version-wrapper flex-column align-items-start">
            <div v-for="(os, osid) in oslist[kind]" :key="osid" class=""
              :class="selected_osid == osid ? 'datacenter-item system-item selected-os' : 'datacenter-item system-item'"
              :osid="osid" @click="selected_osid = osid, selected_os_name = os.name, selected_configid = os.config_id"
              :os-name-iso="os.name" :config-id="os.config_id">
        ...
        <div v-if="products.length > 0">
          <div v-for="(group, key) in product_group" :key="key">
            <div class="row" @click="selected_group_id = key">
              ...
              <div v-for="product in products.filter((p) => p.gid == key)" :key="product.pid" class="col-md-3 m-1 mb-4"
                ...
                <div
                  ...
                  style="cursor: pointer" :product-id="product.pid">
                  ...
                  <div class="server-price mb-4" style="background: none">
                    <span class="price">€<span class="price-value">{{
                  ...
                  <div class="more-details-wrapper mt-3">
                    ...
                    <a class="btn btn-light btn-chevron d-lg-block hover-light-dark toggle-more-detail">{{
                      $t('More_Details') }}</a>
                  </div>
                </div>
              </div>
            ...
            <div class="mb-4">
              ...
              <div style="display: flex" class="mb-4">
                <input type="text" class="form-control" placeholder="••••••••••" id="password" name="password" required
                  disabled v-model="hostpassword" />
                <button type="button" class="btn btn-link" style="padding: 10px" id="randomizeButton_password"
          ...
          <div class="modal-main">
            ...
            <div class="payment-info mt-2">
              <div class="payment-info-review card">
                ...
                <div v-for="(payment_method, key) in payment_methods" :key="key" class="form-check">
                  <input type="radio" class="form-check-input paymentMethod" name="paymentMethod" v-model="paymentmethod"
                    :value="payment_method.module" :checked="key == 0" />
                  <label class="form-check-label" :for="'paymentMethod' + key">{{ payment_method.displayname }}</label>
                </div>
                ...
                <div class="form-check">
                  <input type="radio" class="form-check-input paymentMethod" name="paymentMethod" v-model="paymentmethod"
                    value="credit" />
                  <label class="form-check-label" for="paymentMethod">{{ $t('Account_Funds') }} ({{ $t('available') }} €{{
                    user.credit }})</label>
                </div>
              </div>
            </div>

            <button class="btn-dark d-block w-100 mt-5" id="continue-order" @click="clickCheckout()">
              {{ $t('CheckOut') }}
            </button>
          </div>
        </div>
